...
export default function LocaisVisitadosScreen() {
  const [visitados, setVisitados] = useState<Estabelecimento[]>([]);
  ...
  useEffect(() => {
    const fetchVisitados = async () => {
      try {
        ...
        const response = await api.get('/visitados/me');
        setVisitados(response.data);
      } catch (error) {
        console.error("Erro ao buscar locais visitados:", error);
        Alert.alert("Erro", "Não foi possível carregar seus locais visitados.");
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>Você ainda não marcou nenhum local como visitado.</Text>
          </View>
        }
      />
    </SafeAreaView>
